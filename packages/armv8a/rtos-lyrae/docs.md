<!-- %title eChronos RTOS Manual: Lyrae Variant -->
<!-- %version 0.2 -->
<!-- %docid fssd45 -->
...
Throughout this document, *eChronos RTOS* or *the RTOS* will refer specifically to the *Lyrae* variant of the eChronos RTOS.
...
The Lyrae variant is designed to support system architectures that include multiple CPU cores, and multiple software sub-systems that require pre-emptive, priority based scheduling.
The top-level concept used to support multiple sub-systems is a *task-group*.
A task-group is configured to run on a specific (fixed) CPU core at a specific (fixed) priority.
The task-group scheduler ensures that the highest priority, runnable task-group for a specific CPU core shall always run in preference to any lower-priority task-group.
The scheduler does not allow for multiple task-groups running at the same priority.

Within a task-group the functionality provided by the RTOS matches that which is available in the *Rigel* RTOS-variant.
Individual tasks execute within a specific task-group (there is no facility for moving tasks between task-groups).
Within the task-group individual task are scheduled using a non-preemptive, round-robin scheduler.
...
They include [Interrupt Service Routines], [Signals], [Time and Timers], and [Mutexes].

A distinctive feature of the RTOS is that these objects, including task-groups and tasks, are defined and configured at build time (see [Configuration Reference]), not at run time.
...
The [<span class="api">start</span>] API should be called on each CPU core.
...
To support isolated sub-systems the RTOS provides the task-group abstraction.
Each task-group is specified to run on a specific CPU core at a specific priority.
Each of the other RTOS objects including [Tasks], [Signals], [Timers], and [Mutexes] are configured as part of a specific task-group.
These objects are isolated from each other; a task in one task-group is not able to signal a task in a different task-group, nor attempt to lock a mutex from a different task-group.

It is possible to specify task groups that have a single task and other RTOS objects.
Such a design may be appropriate when re-using existing software with minimal changes.

## Tasks

Within task-groups tasks form the building blocks for structuring complex sub-systems.
...
The RTOS implements this abstraction by providing tasks, which execute within a task-group.
Each task in the task-group executes its own loop, and the RTOS provides the underlying mechanism for switching the CPU from one task to another.
...
Task-group scheduling is preemptive.
This means that if a high-priority task-group becomes runnable when a lower priority task-group is currently executing there shall be a task-group context switch, and the high-priority task-group becomes active.

A high-priority task-group can become active if a task-group event is raised by a currently running task-group (on either the same or a different CPU core).
Additionally, a task-group may become active due to a timer expiring a an interrupt-event being raised by an interrupt service routine.

### Tasks

Task scheduling is non-preemptive, which means that any task context switch (see [Task Switching]) is actively caused by code that the current task executes.
...
Therefore, to ensure that the task-group as a whole operates correctly, it is important that tasks perform context switches on a regular basis[^context_switch_regularity].

[^context_switch_regularity]: The exact definition of regular in this context depends on the system timing attributes.

For example, if there are any runnable tasks, it is important that they have the opportunity to become the current task within the task-group.
When tasks are frequently interacting with other entities this is usually not a problem, as the task regularly block, providing the other runnable tasks in the task-group with an opportunity to become the current task within the task-group.
...
Task-groups as scheduled on a strict priority basis on each CPU core.
Each task-group is assigned a priority at configuration time.
Additionally each task-group is assigned a CPU core at configuration time.

Each CPU core is scheduled independently.
The scheduler picks the task-group with the highest priority from all runnable task-groups assigned to the CPU core.
If no task-groups are runnable on the CPU core the CPU core will sleep until an interrupt occurs.
A task-group is runnable if any of the tasks within the task-group are runnable.

### Tasks

The scheduling algorithm is an important component of the RTOS because it determines which one of the runnable tasks within a task-group is selected for active execution.
...
*Global signals* are signals which are handled in multiple different tasks within the task-group whereas a *task signal* is handled only by a small fixed set of tasks known at configuration time.
...
A task-group can be configured with a number of interrupt events.
...
Interrupt service routines are not associated with a specific task-group and can raise any interrupt event configured for the CPU core on which the interrupt service routines executes.
...
Task-group events provides the bridge between different task-groups.
A task-group can be configured with a number of task-group events.
Each task-group event is associated with a [<span class="api">TaskId</span>] and [<span class="api">SignalSet</span>][^signalset1].
The [<span class="api">TaskId</span>] and [<span class="api">SignalSet</span>] association is done when the system is configured.

[^signalset1]: See the [Signals] section for more details.

A task in different task-group can call the [<span class="api">taskgroup_event_raise</span>] API to raise one of the task-group events configured in the system.
When an task-group event is raised, it causes the associated signal set to be sent to the task associated with the task-group event.
This provides a safe and efficient mechanism for different task-groups to interact.
...
### <span class="api">TaskGroupId</span>

Instances of this type identify specific tasks.
The underlying type is an unsigned integer of a size large enough to represent all tasks[^taskgroup_id_type].
The [<span class="api">TaskGroupId</span>] should generally be treated as an opaque value.
Arithmetic operations should not be used on a [<span class="api">TaskGroupId</span>].

[^taskgroup_id_type]: This is normally a `uint8_t`.

### <span class="api">TASKGROUP_ID_ZERO</span>

This constant has the type [<span class="api">TaskGroupId</span>] and represents the task with the numerically lowest task-group ID in the system.
This can be used in cases where application code wishes to iterate over all task-groups in the system.

### <span class="api">TASKGROUP_ID_MAX</span>

This constant has the type [<span class="api">TaskGroupId</span>] and represents the task with the numerically highest task-group ID in the system.
This can be used in cases where application code wishes to iterate over all task-groups in the system.


### <span class="api">taskgroup_current</span>

<div class="codebox">TaskGroupId task_current(void);</div>

This function returns the task-group ID for the current task-group.
...
## Task-group Event API

### <span class="api">TaskGroupEventId</span>

Instances of this type refer to specific task-group events.
The underlying type is an unsigned integer of a size large enough to represent all interrupt events[^TaskGroupEventId_width].

[^TaskGroupEventId_width]: This is normally a `uint8_t`.

Also refer to the [Platform Task-group Event API] section.

### `TASKGROUP_EVENT_ID_<name>`

These constants of type [<span class="api">TaskGroupEventId</span>] are automatically generated at build time for each task-group event in the system.
Note that `<name>` is the upper-case conversion of the task-group event's name configured through [`taskgroup_events/taskgroup_event/name`].
Applications should treat the numeric values of these constants as opaque values and use them in preference to raw numeric values to refer to interrupt events.

## Platform Task-group Event API

### <span class="api">taskgroup_event_raise</span>

<div class="codebox">void taskgroup_event_raise(TaskGroupEventId event);</div>

The `taskgroup_event_raise` API raises the specified task-group event.

See [Task-group Events] for further information on the effect of raising a task-group event.
